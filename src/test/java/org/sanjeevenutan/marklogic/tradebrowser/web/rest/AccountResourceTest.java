...
import org.sanjeevenutan.marklogic.tradebrowser.repository.AuthorityRepository;
...
public class AccountResourceTest {
	...
	public void setup() {
		...
		ReflectionTestUtils.setField(accountResource, "userRepository",
				userRepository);
		ReflectionTestUtils.setField(accountResource, "userService",
				userService);
		ReflectionTestUtils.setField(accountResource, "mailService",
				mockMailService);

		AccountResource accountUserMockResource = new AccountResource();
		ReflectionTestUtils.setField(accountUserMockResource, "userRepository",
				userRepository);
		ReflectionTestUtils.setField(accountUserMockResource, "userService",
				mockUserService);
		ReflectionTestUtils.setField(accountUserMockResource, "mailService",
				mockMailService);

		this.restMvc = MockMvcBuilders.standaloneSetup(accountResource).build();
		this.restUserMockMvc = MockMvcBuilders.standaloneSetup(
				accountUserMockResource).build();
	}
	...
	public void testGetExistingAccount() throws Exception {
		...
		restUserMockMvc
				...
				.andExpect(
						jsonPath("$.roles").value(AuthoritiesConstants.ADMIN));
	}
	...
	public void testRegisterValid() throws Exception {
		...
		restMvc.perform(
				post("/api/register").contentType(
						...
						TestUtil.convertObjectToJsonBytes(u))).andExpect(
				status().isCreated());
	...
	public void testRegisterDuplicateLogin() throws Exception {
		...
		UserDTO dup = new UserDTO(u.getLogin(), u.getPassword(), u.getLogin(),
				u.getLastName(), "alicejr@example.com", u.getLangKey(),
				u.getRoles());
		...
		restMvc.perform(
				post("/api/register").contentType(
						TestUtil.APPLICATION_JSON_UTF8).content(
						TestUtil.convertObjectToJsonBytes(dup))).andExpect(
				status().is4xxClientError());

		Optional<User> userDup = userRepository
				.findOneByEmail("alicejr@example.com");
	...
	public void testRegisterDuplicateEmail() throws Exception {
		...
		UserDTO dup = new UserDTO("johnjr", u.getPassword(), u.getLogin(),
				u.getLastName(), u.getEmail(), u.getLangKey(), u.getRoles());
	...
	public void testRegisterAdminIsIgnored() throws Exception {
		UserDTO u = new UserDTO("badguy", // login
				...
				"en", // langKey
				Arrays.asList(AuthoritiesConstants.ADMIN) // <-- only admin
															// should be able to
															// do that
		);
