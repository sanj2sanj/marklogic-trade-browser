package org.sanjeevenutan.marklogic.tradebrowser.web.filter.gzip;
...
public final class GZipResponseUtil {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(GZipResponseUtil.class);

	/**
	 * Gzipping an empty file or stream always results in a 20 byte output This
	 * is in java or elsewhere.
	 * <p/>
	 * On a unix system to reproduce do <code>gzip -n empty_file</code>. -n
	 * tells gzip to not include the file name. The resulting file size is 20
	 * bytes.
	 * <p/>
	 * Therefore 20 bytes can be used indicate that the gzip byte[] will be
	...
	public static boolean shouldGzippedBodyBeZero(byte[] compressedBytes,
		...
		if (compressedBytes.length == EMPTY_GZIPPED_CONTENT_SIZE) {
			if (LOG.isTraceEnabled()) {
				LOG.trace("{} resulted in an empty response.",
	 ...
	 * <p/>
	 * <p>
	 * This is need when a gzipped body is returned so that browsers can
	 * properly decompress it.
	 * </p>
	 *
	 * @param response
	 *            the response which will have a header added to it. I.e this
	 *            method changes its parameter
