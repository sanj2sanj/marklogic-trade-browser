package org.sanjeevenutan.marklogic.tradebrowser.web.rest.util;
 ...
 * href="https://developer.github.com/v3/#pagination">Github API</api>, and
...
public class PaginationUtil {
	...
	public static HttpHeaders generatePaginationHttpHeaders(Page page,
		...
		if (offset < page.getTotalPages()) {
			link = "<"
					+ (new URI(baseUrl + "?page=" + (offset + 1) + "&per_page="
							+ limit)).toString() + ">; rel=\"next\",";
		}
		if (offset > 1) {
			link += "<"
					+ (new URI(baseUrl + "?page=" + (offset - 1) + "&per_page="
							+ limit)).toString() + ">; rel=\"prev\",";
		}
		link += "<"
				+ (new URI(baseUrl + "?page=" + page.getTotalPages()
						+ "&per_page=" + limit)).toString()
				+ ">; rel=\"last\","
				+ "<"
				+ (new URI(baseUrl + "?page=" + 1 + "&per_page=" + limit))
						.toString() + ">; rel=\"first\"";
