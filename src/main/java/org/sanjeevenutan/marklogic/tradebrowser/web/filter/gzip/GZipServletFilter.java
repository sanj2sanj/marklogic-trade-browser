...
public class GZipServletFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (!isIncluded(httpRequest) && acceptsGZipEncoding(httpRequest)
			...
			if (log.isTraceEnabled()) {
				log.trace("{} Written with gzip compression",
			...
			// Handle the request
			final GZipServletResponseWrapper wrapper = new GZipServletResponseWrapper(
					httpResponse, gzout);
			...
			boolean shouldGzippedBodyBeZero = GZipResponseUtil
					.shouldGzippedBodyBeZero(compressedBytes, httpRequest);
			boolean shouldBodyBeZero = GZipResponseUtil.shouldBodyBeZero(
					httpRequest, wrapper.getStatus());
			if (shouldGzippedBodyBeZero || shouldBodyBeZero) {
				// No reason to add GZIP headers or write body if no content was
				// written or status code specifies no
				// content
	...
	private boolean isIncluded(final HttpServletRequest request) {
		...
		if (includeRequest && log.isDebugEnabled()) {
			log.debug(
					"{} resulted in an include request. This is unusable, because"
